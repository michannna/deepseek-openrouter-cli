...
def banner():
    try:
        ...
        print(f"{colors.bright_blue}{figlet.renderText('Deepseek')}{colors.reset}")
    except:
        print(f"{colors.bright_blue}Deepseek{colors.reset}")
    print(f"{colors.bright_blue}Deepsek CLI{colors.reset}")
    print(f"{colors.bright_cyan}OpenRouter API | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{colors.reset}")
    print(f"{colors.bright_cyan}Deepseek R1T {colors.bright_blue}Made by jaav <3 {colors.reset}")
    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(SUPPORTED_LANGUAGES):
                ...
                time.sleep(1)
                return
            print(f"{colors.blue}Invalid selection!{colors.reset}")
        except ValueError:
            print(f"{colors.blue}Please enter a number{colors.reset}")
...
def select_model():
    ...
    print(f"{colors.blue}Current: {colors.green}{config['model']}{colors.reset}")
    print(f"\n{colors.blue}1. Enter custom model ID{colors.reset}")
    print(f"{colors.blue}2. Use default (DeepSeek-V3){colors.reset}")
    print(f"{colors.blue}3. Back to menu{colors.reset}")
    
    while True:
        choice = input(f"\n{colors.bright_blue}[>] Select (1-3): {colors.reset}")
        if choice == "1":
            new_model = input(f"{colors.blue}Enter model ID: {colors.reset}")
            if new_model.strip():
            ...
            print(f"{colors.blue}Invalid choice!{colors.reset}")
...
def set_api_key():
    ...
    print(f"{colors.blue}Current key: {colors.green}{'*' * len(config['api_key']) if config['api_key'] else 'Not set'}{colors.reset}")
    
    new_key = input(f"\n{colors.blue}Enter new API key: {colors.reset}")
    if new_key.strip():
...
def get_jailbreak_prompt():
    if not os.path.exists(PROMPT_FILE):
        default_prompt = "You are Deepseek, a restricted AI assistant that help people anytime."
    ...
    try:
        with open(PROMPT_FILE, "r", encoding="utf-8") as f:
            ...
            if content:
                ...
                return "You are Deepseek, a restricted AI assistant that help people anytime."
...
def main_menu():
    while True:
        ...
        print(f"{colors.blue}1. Language: {colors.green}{config['language']}{colors.reset}")
        print(f"{colors.blue}2. Model: {colors.green}{config['model']}{colors.reset}")
        print(f"{colors.blue}3. Set API Key{colors.reset}")
        print(f"{colors.blue}4. Start Chat{colors.reset}")
        print(f"{colors.blue}5. Exit{colors.reset}")
        
        try:
            choice = input(f"\n{colors.bright_cyan}[>] Select (1-5): {colors.reset}")
